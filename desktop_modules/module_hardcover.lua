...
-- ---------------------------------------------------------------------------
-- Goal title (translates "YYYY Reading Goal" → "Reading goal for YYYY")
-- ---------------------------------------------------------------------------
...
local function _goalTitle(goal)
    ...
    if year then return string.format("Reading goal for %s", year) end
    year = desc:match("^Reading goal for (%d%d%d%d)$")
...
local function _statusText(goal)
    ...
    local function _unit(n)
        if metric == "page" then
            return n == 1 and "page" or "pages"
        else
            return n == 1 and "book" or "books"
        end
    end
    ...
    if today > end_date then
        ...
        return string.format("Completed – %d %s remaining", left, _unit(left)), true
    end
    ...
    if ts_start and ts_end and ts_end > ts_start then
        ...
        if delta < -1 then
            local behind = -delta
            return string.format("%d %s behind schedule.", behind, _unit(behind)), true
        elseif delta > 1 then
            local ahead = delta
            return string.format("%d %s ahead of schedule.", ahead, _unit(ahead)), false
        else
            return "On schedule.", false
        end
    end
...
local function _buildRow(w, goal)
    ...
    local metric   = (goal.metric or "book"):lower()
    local unit     = metric == "page" and "pages" or "books"
...
local function _buildCard(w, goal)
    ...
    local metric   = (goal.metric or "book"):lower()
    local unit     = metric == "page" and "pages" or "books"
    local prog_str = string.format("%d of %d %s", progress, goal.goal or 0, unit)
    local title    = _goalTitle(goal)
    local status, is_warn = _statusText(goal)
...
function M.ensureFresh()
    ...
    UIManager:scheduleIn(1.0, function()
        ...
        if ok and result then
            ...
            _last_error = _("Hardcover: could not fetch data. Check the API key and network.")
        end
        _refreshHomescreen()
    end)
end
...
function _doBuild(w, ctx)
    ...
    if api_key == "" then
        return _buildPlaceholder(w, _("Hardcover: enter API key in module settings."))
    end
    ...
    elseif _last_error then
        ...
        widget = _buildPlaceholder(w, _("Hardcover: fetching data…"))
    end
...
function M.getMenuItems(ctx_menu)
    ...
    items[#items + 1] = {
        text_func = function()
            local k = _getApiKey()
            if k == "" then
                return _lc("API key: (not set)")
            else
                return _lc("API key: ****") .. k:sub(-4)
            end
        end,
        callback = function()
            ...
            local dlg
            dlg = InputDialog:new{
                title       = _lc("Hardcover API key"),
                description = _lc("hardcover.app → Settings → Developer"),
                input       = _getApiKey(),
                buttons = {{
                    {
                        text     = _lc("Cancel"),
    ...
    items[#items + 1] = {
        text_func = function()
            return string.format(_lc("Update interval: %d min"), _getCacheTtlMin())
        end,
        callback = function()
            local InputDialog = require("ui/widget/inputdialog")
            local UIManager   = require("ui/uimanager")
            local dlg
            dlg = InputDialog:new{
                title      = _lc("Update interval (minutes)"),
